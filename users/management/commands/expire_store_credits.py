...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"[DRY-RUN] Found {count} expired credit grants totaling Rs.{total_amount} across matching wallets."
                )
            )
            return
        ...
        if user_id:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully expired {len(debits)} credit grants totaling Rs.{total_expired} for user {user.email}."
                    )
                )
